use std::{rc::Rc,rc::Weak, cell::RefCell, ops::Deref, borrow::Borrow};
use super::compartment::Compartment;

#[derive(Debug)]
pub struct GeneSys{
    pub storage: Vec<Weak<RefCell<Compartment>>>,
            ...
            .as_str() == par {
                for sub_comp in self.storage.iter(){
                    ...
                    .name.as_str() == sub{
                        ...
                        .unwrap()
                        .borrow_mut()
                        .par_comp = Some(par_compart.clone());
                        
                        par_compart.clone()
                        .upgrade()
                        .unwrap()
                        .borrow_mut()
                        .sub_comp.push(Some(sub_comp.clone()));
